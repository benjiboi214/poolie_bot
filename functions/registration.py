...
def confirm_close_registration(event, context):
    print("confirm_close_registration: Receieved event - ", event)
    
    event['generate_draw'] = False
    with DatabaseManager("confirm_close_registration") as db:
        user = User.select().where((User.slack_id == event["user"]["id"])).first()
        competition = db.get_active_competition(event['actions'][0]["value"])

        if competition and competition.status == Competition.REGISTERING:
            if user_is_admin_of_competition(user, competition):
                # Update the competition to trigger next action flow
                ...
                response_message['attachments'][0]['footer'] = "Competition registration closed, generating draw."

                print("confirm_close_registration: Replacing action message with: ", response_message)
...
def cancel_close_registration(event, context):
    print("confirm_close_registration: Receieved event - ", event)
    
    with DatabaseManager("cancel_close_registration") as db:
        user = User.select().where((User.slack_id == event["user"]["id"])).first()
        competition = db.get_active_competition(event['actions'][0]["value"])

        if competition and competition.status == Competition.REGISTERING:
            if user_is_admin_of_competition(user, competition):
                # Don't update the competition. Prompt admin to close again in future.

                response_message = event['original_message']
                del response_message['attachments'][0]['actions']
                response_message['attachments'][0]['footer'] = "Close of registration cancelled."
                ...
                event['action_event'] = {
                    "destination": user.slack_id,
                    "text": "Cancelling close of registration, send the close registration command again to close."
                }
            else:
                # User is not the admin. Advise them.
                event['action_event'] = get_admin_error_message(user, competition)
